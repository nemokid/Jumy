...
export async function POST(request) {
  try {
    ...
    const deleted = await sql`DELETE FROM messages WHERE recipient_hash = ${targetUsernameHash}`;

    await sql`
      INSERT INTO logs (event_type, client_ip, details)
      VALUES ('admin_clear_account', 'admin', ${`cleared ${deleted.rowCount} messages`})
    `;

    return NextResponse.json({ success: true, deletedMessages: deleted.rowCount });
  } catch (error) {
    console.error('Admin clear-account error:', error);
